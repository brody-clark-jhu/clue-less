...
import type { ServerEvent, PlayerState, Character, LobbyPlayer } from "./types";
import {
  View,
  onJoinLobbyClick,
  onBoardClick,
...
export class PlayerController {
  ...
  //lobby-specific state tracked on the client
  ...
  constructor(client: Client, view: View) {
    ...
    //sends character_select command when a card is clicked
    onCharacterSelectClick((character: Character) => {           
      console.log(`Character selected: ${character}`);           
      this.selectedCharacter = character;                     
      this.view.SetCharacterSelected(character);                 
  ...
  private handleServerEvent(event: ServerEvent) {
    ...
    switch (event.type) {
      ...
      // allows both panes to re-render for lobby
      case "lobby_update": {                                                    
        this.lobbyPlayers = event.payload.players;                             
        this.view.UpdateLobbyPlayers(this.lobbyPlayers, this.playerId);  
        break;                                                                
      }                                                                     

      case "character_selected": {                                                  
        console.log(                                                              
          `Character ${event.payload.character} claimed by ${event.payload.playerId}` 
        );                                                                      
        break;                                                                     
      }                                                                          
      //transitions to the game start
      case "game_started": {                                                     
        console.log(`Game started. First player: ${event.payload.startingPlayerId}`);
